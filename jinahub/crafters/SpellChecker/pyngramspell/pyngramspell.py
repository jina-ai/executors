...
class BKTree:
    ...
    def __init__(
        self, distfn: Callable, words: List[str], sort_candidates: bool = False
    ):
        self.distfn = distfn
    ...
    def query(self, query_word: str, n: int, return_distances: bool = False):
        ...
        distance_candidate_list = self._search_descendants(
            self.tree, n, self.distfn, query_word
        )
    ...
    def _correct_with_bigrams(self, tokenized_sentence: List[str]) -> List[str]:
        ...
        def _probability_bigram(word1, word2):
            ...
            if self.bigram_freq_dict.get(bigram, 0) == 0:
                ...
                return (
                    self.bigram_freq_dict.get(bigram, 0) / self.unigram_freq_dict[word1]
                )

        def _interpolation_probability(word1, word2):
            return (1 - self.lambda_interpolation) * _probability_bigram(
                word1, word2
        ...
        for index, word in enumerate(tokenized_sentence):
            if word not in self.vocabulary:
                candidates = {
                    candidate: _interpolation_probability(previous_word, candidate)
                    for candidate in self._get_candidates(word, max_dist=self.max_dist)
                }
