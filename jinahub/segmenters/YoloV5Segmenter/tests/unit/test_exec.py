...
@pytest.mark.gpu
def test_n_detections_gpu(build_da):
    expected_detections = {'bus.jpg': 5, 'zidane.jpg': 3, 'man.jpg': 3}
    da = build_da()
    segmenter = YoloV5Segmenter(
        model_name_or_path=(os.path.join(cur_dir, '../data/models/yolov5s.pt')),
        device='cuda',
    )
    assert segmenter.device == torch.device('cuda')
    segmenter.segment(da, parameters={})
    for doc in da:
        assert len(doc.chunks) == expected_detections[doc.tags['filename']]
