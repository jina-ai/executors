    ...
    def segment(self, docs: DocumentArray, parameters: dict, *args, **kwargs):
        ...
        if docs:
            ...
            for docs_batch in get_docs_batch_generator(
                ...
                blob_batch = [
                    move_channel_axis(d.blob, -1, self._default_channel_axis)
                    for d in docs_batch
                ]
                all_predictions = self._predict(blob_batch)

                for doc, blob, predictions in zip(
                    docs_batch, blob_batch, all_predictions
                ):
                    ...
                    for bbox, score, label in zip(
                        bboxes.numpy(), scores.numpy(), labels.numpy()
                    ):
